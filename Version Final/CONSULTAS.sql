...
SELECT COUNT(e.Id) AS CantidadEquipos FROM (
	...
	INNER JOIN PartidoEquipoLocal pel ON pel.Id_Equipo = e.Id
	INNER JOIN Partido p ON p.Id = pel.Id_Partido
	WHERE YEAR(p.Fecha) = 2022 AND MONTH(p.Fecha) = 12 AND DAY(p.Fecha) = 1
	
	UNION
	
	SELECT e.Id FROM Equipo e
	INNER JOIN PartidoEquipoVisitante pel ON pel.Id_Equipo = e.Id
	INNER JOIN Partido p ON p.Id = pel.Id_Partido
	WHERE YEAR(p.Fecha) = 2022 AND MONTH(p.Fecha) = 12 AND DAY(p.Fecha) = 1
) e
...
INNER JOIN Estadistica e ON e.Id_Jugador = j.Id
INNER JOIN Partido p ON p.Id = e.Id_Partido
...
FROM Partido p
INNER JOIN PartidoEquipoLocal pel ON pel.Id_Partido = p.Id
INNER JOIN Equipo el ON el.Id = pel.Id_Equipo
INNER JOIN PartidoEquipoVisitante pev ON pev.Id_Partido = p.Id
INNER JOIN Equipo ev ON ev.Id = pev.Id_Equipo
...
SELECT TOP(1)
    ...
    eq.Nombre AS Equipo,
    CAST(SUM(e.Valor) AS DECIMAL(5,2)) / COUNT(DISTINCT e.Id_Partido) AS PromedioRobos
FROM Jugador j
INNER JOIN Estadistica e ON e.Id_Jugador = j.Id
INNER JOIN TipoEstadistica te ON te.Id = e.Id_Tipo_Estadistica AND te.Descripcion = 'Robos'
INNER JOIN EquipoJugador ej ON ej.Id_Jugador = j.Id
INNER JOIN Equipo eq ON eq.Id = ej.Id_Equipo
...
SELECT TOP(5)
	p.Equipo,
	CAST(SUM(p.Puntos) AS FLOAT) / COUNT(p.PartidoId) AS PromedioPuntos
FROM (
	SELECT 
		p.Id AS PartidoId, 
		e.Id AS EquipoId,
	...
	FROM Partido p
	INNER JOIN PartidoEquipoLocal pel ON pel.Id_Partido = p.Id
	INNER JOIN Equipo e ON e.Id = pel.Id_Equipo
	...
	FROM Partido p
	INNER JOIN PartidoEquipoVisitante pev ON pev.Id_Partido = p.Id
	INNER JOIN Equipo e ON e.Id = pev.Id_Equipo
) AS p
GROUP BY p.EquipoId, P.Equipo
ORDER BY CAST(SUM(p.Puntos) AS FLOAT) / COUNT(p.PartidoId) DESC
...
INNER JOIN PartidoEquipoLocal pel ON pel.Id_Partido = p.Id
INNER JOIN Equipo el ON el.Id = pel.Id_Equipo
INNER JOIN Ciudad cel ON cel.Id = el.Id_Ciudad
INNER JOIN PartidoEquipoVisitante pev ON pev.Id_Partido = p.Id
INNER JOIN Equipo ev ON ev.Id = pev.Id_Equipo
INNER JOIN Ciudad cev ON cev.Id = ev.Id_Ciudad
WHERE (
...
SELECT 
	CAST(SUM(pel.Puntos + pev.Puntos) AS FLOAT) / COUNT(p.Id)
FROM Partido p 
INNER JOIN PartidoEquipoLocal pel ON pel.Id_Partido = p.Id
INNER JOIN Equipo el ON el.Id = pel.Id_Equipo
INNER JOIN Division dl ON dl.Id = el.Id_Division
INNER JOIN PartidoEquipoVisitante pev ON pev.Id_Partido = p.Id
INNER JOIN Equipo ev ON ev.Id = pev.Id_Equipo
INNER JOIN Division dv ON dv.Id = ev.Id_Division
GROUP BY el.Id_Division
